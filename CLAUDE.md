...
- **Parser `markdownToHtml` dans `build.js` est maison aussi**. Listes : reconnaît `-`, `*`, `+` (puces) et `1.` (numérotées). Blockquote `>`, headings `##`, gras `**`, italique `*` et `_`, gras-italique `***`, liens `[]()`, notes de bas de page `[^cle]` / `[^cle]: texte`. Cas non couverts : code blocks ` ``` `, listes imbriquées, images `![]()`. `markdownToHtml(md, slug, lang)` retourne `{ body, notesHtml }`, pas une string.
- **Échappements `\*`, `\_`, `\#`…** : Sveltia insère le backslash tout seul quand l'auteur tape un caractère de ponctuation markdown littéral. `inlineMarkdown` met ces séquences de côté derrière un sentinelle ` n ` avant toutes les règles et les restaure après. Sans ça, le backslash sortait visible en production (arrivé le 2026-08-03 sur `enquete-harcelement`).
- **Ordre des règles dans `inlineMarkdown` (fragile)** : `***` avant `**` avant `*` (sinon le gras découpe `***x***` en `<strong>*x</strong>*`) ; liens et notes **après** l'échappement des guillemets, pour que le libellé soit déjà échappé et que l'échappement ne touche pas les balises générées. Les URL absolues reçoivent `target="_blank"`, pas les ancres internes ; `javascript:` est neutralisé.
- **Le gras est presque invisible** : `.article-body p` est en `font-weight: 300`, `<strong>` résout à 400 pour DM Sans. Non corrigé — la correction (`font-weight: 500`) change l'apparence des articles déjà publiés.
- **Notes de bas de page** : les IDs sont préfixés par le slug (`fn-<slug>-1`) parce que tous les articles cohabitent comme panneaux dans le même `index.html`. La numérotation suit l'ordre de première apparition dans le texte, pas l'ordre de définition. Le clic est intercepté en JS (défilement interne au panneau) pour ne pas écraser le hash `#article-<slug>` utilisé par le partage et le deep-link.
- **`.article-body ul li` ne doit pas repasser en `display: flex`** : en flexbox chaque élément enfant devient son propre item, donc un `<strong>` ou un `<sup>` dans une puce était éjecté en colonne avec la `gap` au milieu de la phrase. La puce est en `position: absolute` depuis 2026-08-03.
